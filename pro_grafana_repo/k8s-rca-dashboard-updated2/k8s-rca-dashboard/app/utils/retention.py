...
def run_retention():
    ...
    try:
        ...
        # Hard cap: Keep only the 15 most recent records of each type in the database
        for Model, time_col in [
            (RestartHistory, RestartHistory.timestamp),
            (K8sEvent, K8sEvent.last_seen),
            (RootCauseRecord, RootCauseRecord.timestamp),
            (Alert, Alert.timestamp)
        ]:
            top_ids = [r[0] for r in db.query(Model.id).order_by(time_col.desc()).limit(15).all()]
            if top_ids:
                n = db.query(Model).filter(~Model.id.in_(top_ids)).delete(synchronize_session=False)
                deleted[f"{Model.__tablename__}_capped"] = n
