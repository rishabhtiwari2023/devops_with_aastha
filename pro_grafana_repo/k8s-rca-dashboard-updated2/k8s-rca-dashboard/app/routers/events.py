...
from app.utils.serializers import row_to_dict, rows_to_list, paginate_query, paginate_list
...
def list_events(
    ...
    q = q.order_by(K8sEvent.last_seen.desc())
    return paginate_query(q, page=page, page_size=page_size, fallback_limit=limit)
...
def list_restarts(
    ...
    q = q.order_by(RestartHistory.timestamp.desc())
    return paginate_query(q, page=page, page_size=page_size, fallback_limit=limit)
...
def timeline(
    ...
    items = items[:15]
    return paginate_list(items, page=page, page_size=page_size)
