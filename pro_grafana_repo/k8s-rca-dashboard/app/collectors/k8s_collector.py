...
class KubernetesCollector:
    ...
    async def collect_once(self):
        ...
        db = SessionLocal()
        try:
            await asyncio.to_thread(self._collect_nodes, db)
            await asyncio.to_thread(self._collect_pods, db)
            await asyncio.to_thread(self._collect_events, db)
            ...
            logger.exception("Unexpected error in Kubernetes collector")
            db.rollback()
        finally:
            db.close()
    ...
    # Nodes
    # ------------------------------------------------------------------
    def _collect_nodes(self, db: Session):
    ...
    # Pods + Containers
    # ------------------------------------------------------------------
    def _collect_pods(self, db: Session):
    ...
    def _deployment_for_replicaset(self, namespace: str, rs_name: str) -> str:
        ...
        deployment_name = ""
        try:
            rs = self.apps.read_namespaced_replica_set(rs_name, namespace)
            for ref in (rs.metadata.owner_references or []):
                if ref.kind == "Deployment":
                    deployment_name = ref.name
                    break
        except ApiException as e:
            logger.debug("Could not resolve ReplicaSet %s/%s -> Deployment: %s",
                         namespace, rs_name, e)

        self._rs_to_deployment_cache[key] = deployment_name
        return deployment_name

    # ------------------------------------------------------------------
    # Events
    # ------------------------------------------------------------------
    def _collect_events(self, db: Session):
        events = self.core.list_event_for_all_namespaces(watch=False)
        ...
        for ev in events.items:
            ...
            if involved and involved.kind == "Pod":
                ...
                pod_row = db.query(Pod).filter(Pod.name == involved.name,
                                                Pod.namespace == ev.metadata.namespace).one_or_none()
                if pod_row:
    ...
    def _update_probe_failure_counts(self, db: Session):
        ...
        for (namespace, pod_name), counts in tally.items():
            pod_row = db.query(Pod).filter(Pod.name == pod_name,
                                            Pod.namespace == namespace).one_or_none()
            if not pod_row:
                continue
