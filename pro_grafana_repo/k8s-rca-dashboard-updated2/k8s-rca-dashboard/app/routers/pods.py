...
GET /api/pods/failing            — pods that are failing right now, each with
                                    its RCA reason attached (NEW) — "why did
                                    my pod fail?" in one call.
GET /api/pods/{uid}              — single pod detail + latest metrics +
                                    latest RCA root-cause reason (enriched)
...
GET /api/pods/{uid}/root-cause   — latest RCA verdict for this pod alone (NEW)
"""
...
def list_pods(
    ...
    if page is not None:
        ...
        total = q.count()
        pods = q.order_by(Pod.namespace, Pod.name).offset((page - 1) * page_size).limit(page_size).all()
    else:
        total = q.count()
...
def failing_pods(
    ...
    """
    NEW: Every pod currently Failed/Unknown or Running-but-not-ready, each
    annotated with its latest RCA reason (category, severity, short label,
    full explanation). This is the direct answer to "when a pod fails,
    tell me the reason" — poll this, or watch for WebSocket `type: alert`
    frames which carry the same shape the moment the RCA engine detects it.
    """
    pods = (
        db.query(Pod)
        .filter(
        ...
        .order_by(Pod.namespace, Pod.name)
        .limit(limit)
        .all()
    )
    if not pods:
    ...
    subq = (
        ...
        .filter(RootCauseRecord.pod_uid.in_(uids))
    ...
    for pod in pods:
        d = pod.to_dict()
        rca = rca_by_uid.get(pod.uid)
        d["root_cause"] = row_to_dict(rca) if rca else None
        d["reason_available"] = rca is not None
        result.append(d)
    return result
...
def pod_detail(uid: str, db: Session = Depends(get_db)):
    """Full pod record: containers, latest metrics, recent events, and
    (NEW) the most recent RCA root-cause verdict for this pod, if any —
    so the pod detail panel can show *why* it failed without a second
    request."""
    ...
    # NEW: attach the latest RCA root-cause verdict for this pod, if any
    latest_rca = (
        db.query(RootCauseRecord)
    ...
    d["root_cause"] = row_to_dict(latest_rca) if latest_rca else None
    d["reason_available"] = latest_rca is not None
    return d


@router.get("/{uid}/root-cause")
def pod_root_cause(uid: str, db: Session = Depends(get_db)):
    """NEW: Latest RCA verdict for a single pod, in isolation (lightweight
    version of the field already embedded in GET /api/pods/{uid})."""
    _require_pod(uid, db)
    latest_rca = (
        db.query(RootCauseRecord)
        .filter(RootCauseRecord.pod_uid == uid)
    ...
    if not latest_rca:
        return {"reason_available": False, "root_cause": None}
    return {"reason_available": True, "root_cause": row_to_dict(latest_rca)}
